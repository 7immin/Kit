...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS rooms (
    ...
    ai_notes_generated INTEGER DEFAULT 0,           -- 현재 대본 기준으로 AI 노트를 이미 생성했는지 (재클릭으로 API를 낭비하지 않도록 버튼 비활성화 여부의 근거). 대본을 다시 업로드하면 0으로 초기화됨
...
try {
  db.exec(`ALTER TABLE rooms ADD COLUMN ai_notes_generated INTEGER DEFAULT 0`);
} catch (e) {
  // 이미 컬럼이 있으면(신규 DB) 무시
}
