...
| 분류 | 사용 기술 |
|---|---|
| 핵심 기술 | - Node.js + Express 5(REST API), Socket.io 4(실시간 통신)<br> - React 19 + Vite(PC/청중 웹)<br> - React Native + Expo SDK 54(모바일 앱) |
| 실행 환경 | - 서버: Node.js(Railway)<br> - 웹: Vercel<br> - 모바일 앱: Expo |
| 데이터 저장 | - SQLite(`better-sqlite3`)<br> - Railway Volume |
| 외부 API / 서비스 | - Google Gemini API(`@google/generative-ai`, `gemini-3.1-flash-lite`)<br> - Railway(백엔드 배포)<br> - Vercel(웹페이지 배포)<br> - Expo(모바일 앱 배포) |
| 기타 | - 인증: `jsonwebtoken`(JWT) + `bcrypt`(비밀번호 해시)<br> - 업로드: `multer`, `pdf-to-png-converter`, `mammoth`(DOCX 파싱)<br> - QR 코드 생성: `qrcode.react`<br> - 라우팅: `react-router-dom` / `expo-router` |
